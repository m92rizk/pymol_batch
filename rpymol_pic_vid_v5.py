...
parser.add_option("", "--mtz-map", dest="mtz_map", default="")
parser.add_option("", "--rmsd_2fofc", dest="rmsd2fofc", default="1")
parser.add_option("", "--rmsd_fofc", dest="rmsdfofc", default="3")
...
if options.mtz_map != "" :
    ...
    mtz_map=os.path.basename(options.mtz_map)
    mtz_map_dir=os.path.dirname(options.mtz_map)
...
for folder in sorted(paths):
    ...
    try:
        ...
        comp=""
    if options.mtz_map == "" :
        if os.path.isfile(str(folder)+'/'+str(maptype)+'map.mtz') and options.overwrite == False : 
            print(' '+str(maptype)+'map.mtz already exists')
            shutil.copyfile(str(folder)+'/'+str(maptype)+'map.mtz', str(maptype)+'map.mtz')
        else:
...
# prepare .pym file
    ...
    if options.origin != " " :
        ...
        originname=str(maptype)+'refmacout_'+str(pdbname)
        with open('pym_'+str(LIG)+'.pml', 'a') as outpm:
            outpm.write('load '+str(originname)+'.pdb\nsuper '+str(pdbname)+', '+str(originname)+'\ndelete '+str(originname)+'\n')
    with open('pym_'+str(LIG)+'.pml', 'a') as outpm:
...
# start pymol
    ...
    if options.photo is True:
        ...
        if dot == ".":
            #print('============================================\n exporting image')
            shutil.copyfile('image_'+str(LIG)+'-'+str(os.path.basename(folder))+str(maptype)+'.png', str(pathlib.Path(folder).parent)+'/image_'+str(LIG)+'-'+str(os.path.basename(folder))+'_'+str(maptype)+str(comp)+'.png')
        else:
            #print('============================================\n exporting image')
            shutil.copyfile('image_'+str(LIG)+'-'+str(os.path.basename(folder))+str(maptype)+'.png', str(pathlib.Path(folder))+'/image_'+str(LIG)+'-'+str(maptype)+str(comp)+'.png')
