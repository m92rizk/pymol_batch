...
parser.add_option("-z", "--surface", dest="surface", default="0",help="directory min depth to search in")
parser.add_option("-i", "--ignore", dest="ignore", default="",help="directory to ignore")
...
parser.add_option("", "--cif", dest="cif")
...
if options.lig is None:
    lig = "nan" # input ("Enter ligand (or amino acid) name: ")
else:
    lig = options.lig
    ...
    for mypath, dirs, files, in os.walk(mypath):
        ...
        for subdir in dirs:
            ...
            if options.ignore == "" :
                ...
                index = mypath.find(options.ignore)
            if (dirname.startswith(options.directory)) and (sub_path >= (orig_path + int(options.surface))):
                if (index == -1):
                #print(str(orig_path)+"\t"+str(options.depth)+"\t<=  "+str(sub_path))
                    print(os.path.join(mypath,subdir)+"\t"+str(sub_path))
...
#                                                          CIF FILE
if options.cif:
    cif_file=os.path.abspath(options.cif)
    cif=" --cif "+str(cif_file)
else:
    cif=" "
...
for folder in sorted(paths):
    ...
    if (options.fft_map == "") and (options.user_mtz == ""):
        if os.path.isfile(str(folder)+'/'+str(maptype)+str(mtz_map)) and options.overwrite == False : 
            ...
            if os.path.isfile(str(folder)+'/junk_xdsconv.mtz') and options.overwrite == False :
...
with open("rpymol_command_lines.log", "a") as outlog:
    outlog.write("==================================\n")
    outlog.write("DATE: "+str(date)+"\n")
    outlog.write("Time: "+str(current_time)+"\n")
    outlog.write("Command line: "+str(command_line)+"\n")
    outlog.write("==================================\n")    
